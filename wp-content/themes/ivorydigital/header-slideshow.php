		...
		<div id="bg-slideshow" class="flexslider">
			...
			if($flamingo_slideshow != '' && count($flamingo_slideshow)>0){
			    foreach($flamingo_slideshow as $flamingo_slide){ 
			        ...
			        if($flamingo_slide['description']){ $slide_desc = '<div class="slide-desc">'.$flamingo_slide['description'].'<br /><a class="wpb_button_a" title="" href="'.get_permalink($link->ID).'"><span class="wpb_button  wpb_wpb_button wpb_regularsize" >View Project</span></a></div>'; } else { $slide_desc=''; }
			    ?>
